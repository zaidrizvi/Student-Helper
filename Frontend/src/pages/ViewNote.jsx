    ...
    <PageShell>
      ...
      <AnimatePresence>
        {isShareModalOpen ? <>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }} className="fixed left-1/2 top-1/2 z-[70] max-h-[calc(100vh-2rem)] w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto"><Panel className="p-5 sm:p-7"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><SectionLabel>Share note</SectionLabel><h2 className="mt-2 font-display text-xl font-bold text-slate-950 dark:text-white sm:text-2xl">Public study guide link</h2></div><button onClick={() => setIsShareModalOpen(false)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-500 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300"><X className="h-5 w-5" /></button></div><p className="mt-4 text-sm leading-7 text-slate-500 dark:text-slate-400">Anyone with this link can read the AI study guide and download its PDF, but they will not see the raw extracted source text.</p><div className="mt-5 rounded-[24px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900"><div className="flex items-start justify-between gap-4"><div className="min-w-0 flex-1"><p className="text-xs uppercase tracking-[0.18em] text-slate-400">{isSharing ? "Generating link" : "Share URL"}</p><p className="mt-2 break-all text-sm text-slate-600 dark:text-slate-300">{isSharing ? "Please wait while the link is created..." : shareUrl || "No link generated yet."}</p></div>{!isSharing && shareUrl ? <button onClick={() => handleCopy(shareUrl, "share")} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-500 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">{copiedId === "share" ? <Check className="h-4 w-4 text-emerald-500" /> : <Copy className="h-4 w-4" />}</button> : null}</div></div>{note?.share?.expiresAt ? <div className="mt-4 rounded-[24px] bg-slate-50 p-4 text-sm text-slate-500 dark:bg-slate-900 dark:text-slate-400"><div className="flex items-center gap-2"><Globe className="h-4 w-4 text-sky-600 dark:text-sky-300" />Link expires {formatDateTime(note.share.expiresAt)}</div></div> : null}<AnimatePresence>{shareError ? <div className="mt-4"><InlineMessage message={shareError} tone="error" /></div> : null}</AnimatePresence><div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><Button variant="secondary" onClick={handleShare} disabled={isSharing} className="w-full sm:w-auto">{isSharing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}{shareUrl ? "Refresh link" : "Generate link"}</Button><Button variant="ghost" onClick={handleRevokeShare} disabled={isRevokingShare || !note?.share?.isActive} className="w-full sm:w-auto">{isRevokingShare ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldOff className="h-4 w-4" />}Revoke link</Button></div></Panel></motion.div>
        </> : null}
      </AnimatePresence>
    </PageShell>
  );
